...
char *get_client_operation(int admin) {
    ...
    char *param = malloc(400*sizeof(char));     // Parâmetro(s) para a operação
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        if (msg == NULL) { // Client ão deseja fazer mais nenhuma requisição
            break;
        }

        int len = strlen(msg);                  // Tamanho da mensagem recebida
        ...
        free(msg); // Libera a memória alocada para a mensagem enviada
        ...
        do { // Garante que todos os bytes serão recebidos ou que a operação será abortada
            ...
            if (numReady == -1) { // Erro de polling
                ...
                // Lê quantos bytes foram enviados, lembrando que os pacotes podem vir fora de ordem
                if (bytes_to_be_received == 0) {
                    // Verifica quantos bytes foram recebidos, através do primeiro inteiro depois do número do pacote e do espaço adicionado
                    bytes_to_be_received = strtol(buf+dismissed_bytes, NULL, 10);
                    // String que armazena o tamanho da mensagem
                    snprintf(message_size_string, 20, "%ld ", bytes_to_be_received);
                }

                // Contém o número de bytes a ser desprezado (tamanho da mensagem e número do pacote)
                dismissed_bytes += strlen(message_size_string);
                ...
                if (datagram_number < 10) {
                    ...
                    /* Caso o número de datagramas seja maior ou igual a 10 e menor que 100, ele adicionou 1 byte de cabeçalho 
                       a menos nos 10 primeiros datagramas. Por isso, adicionamos esse 10, para compensar essa diferença, 
                       já que o dismissed bytes agora é 1 número maior que para datagramas de número menor que 10 */
                    datagram_position = datagram_number * (total-dismissed_bytes) + 10;
                } else {
                    datagram_position = datagram_number * (total-dismissed_bytes) + 10 + 100;
                }

                // Desconsidera os bytes extras de quando a mensagem é menos que o buffer
                if ((received_bytes+numbytes-dismissed_bytes) > bytes_to_be_received) {
                    numbytes -= received_bytes + numbytes - dismissed_bytes - bytes_to_be_received;
                    received_message[bytes_to_be_received] = '\0';
                }

                /* Copia os bytes recebidos para buffer final que armazena toda a mensagem, descartando o tamanho da mensagem, 
                   o número do pacote e o espaço adicionado */
        ...
        if (!timeout && received_bytes > 0) {
            if (option == 6 && received_bytes > 60) {
                ...
                fwrite(received_message, bytes, 1, fp); // Armazena a imagem no arquivo criado
