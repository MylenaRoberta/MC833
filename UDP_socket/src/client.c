    ...
    while (1)
    {
        ...
        { // Garante que todos os bytes serão recebidos ou que a operação será abortada
            ...
            else if (fds[0].revents & POLLIN)
            {
                ...
                if (received_bytes + numbytes - dismissed_bytes > bytes_to_be_received) // Para desconsiderar os bytes extras de quando a mensagem é menos que o buffer
                {
        ...
        if (!timeout && received_bytes > 0)
        {
            if (option == 6)
            {
                ...
                fwrite((uchar *)received_message, bytes, 1, fp); // Armazena a imagem no arquivo criado
